...
// Estratégia: em vez de injetar $q.notify na instância (acoplamento interno),
// mockamos a factory erebusMessage para espiar os métodos semânticos.
...
describe('CharacterPage.vue', () => {
  beforeEach(() => {
    ...
    mockAttributes.value = {
      FR: 10, DEX: 10, AGI: 10, CON: 10, INT: 10, WILL: 10, CAR: 10, PER: 10,
    };
  ...
  // ── handleSubmit ───────────────────────────────────────────────────────────
  // Estratégia: mockamos erebusMessage() via vi.mock para espiar success/danger.
  // Não usamos injectNotifySpy — o acoplamento ao $q.notify foi removido.

  describe('handleSubmit', () => {
    it('submit() resolve true → erebusMessage().success é chamado', async () => {
      mockCanSave.value = true;
      mockSubmit.mockResolvedValue(true);
      const wrapper = mountPage();
      await nextTick();
    ...
    it('submit() resolve false → erebusMessage().danger é chamado', async () => {
      mockCanSave.value = true;
      ...
      expect(mockDanger).toHaveBeenCalledOnce();
    });

    it('submit() lança exceção → erebusMessage().danger é chamado', async () => {
      mockCanSave.value = true;
